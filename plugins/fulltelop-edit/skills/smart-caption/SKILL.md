...
- 実測（DH木村さん 2026-07）: 49枚中21枚を +0.17〜+0.74s 補正、以後のズレ報告ゼロ
...
| `snap_telop_timing.py` | VAD ベースのタイミング吸着（retime の旧経路・フォールバック用）。START_OFFSET=0.18 は「発話の少し後に出す」流儀（しゅうへい好み）| （現在は補助） |
